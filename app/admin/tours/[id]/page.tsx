...
export default function EditTour() {
  ...
  const fetchTour = async () => {
    try {
      setLoading(true);
      const data = await apiGet<Tour>(`/api/tours/${id}`);
      setFormData(data);
    } catch (err: any) {
  ...
  const handleRemoveArrayItem = (field: string, index: number) => {
    setFormData(prev => ({
      ...prev,
      [field]: (prev[field as keyof Tour] as any[]).filter((_, i) => i !== index),
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiPut(`/api/tours/${id}`, formData);
                ...
                <div className="space-y-2">
                  {(formData.includeItems || []).map((item, index) => (
                    <div key={index} className="flex gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => handleRemoveArrayItem('includeItems', index)}
                ...
                <div className="space-y-2">
                  {(formData.excludeItems || []).map((item, index) => (
                    <div key={index} className="flex gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => handleRemoveArrayItem('excludeItems', index)}
                        className="px-3 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded transition"
                      >
                        <X size={16} />
                      </button>
