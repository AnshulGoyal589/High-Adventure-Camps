...
export default function ActivitiesAdmin() {
  ...
  const fetchActivities = async () => {
    try {
      setLoading(true);
      const data = await apiGet<ActivityType[]>('/api/activities');
      setActivities(data);
    } catch (error) {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-xl font-bold mb-1">All Activities</h2>
              <p className="text-muted-foreground">Manage individual adventure activities</p>
            </div>
            <Link href="/admin/activities/create" className="btn-primary flex items-center gap-2">
              <Plus size={20} />
              Add Activity
            </Link>
          </div>
          ...
          {loading ? (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Participants</th>
                    <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {activities.map((activity) => (
                    <tr key={activity._id} className="border-b border-border last:border-b-0 hover:bg-muted/50">
                      <td className="px-4 py-3 font-medium">{activity.title}</td>
                      <td className="px-4 py-3 text-muted-foreground">{activity.duration}h</td>
                      <td className="px-4 py-3 font-semibold text-primary">₹{activity.price.toLocaleString()}</td>
            ...
            <div className="text-center py-12 bg-muted/30 rounded-lg">
              ...
              <Link href="/admin/activities/create" className="btn-primary">
